            ...
            if (cmd_type == BI_CMD_EXIT) {
                ...
                if (cmd_list.commands[0].argc > 1) {
                    if (chdir(cmd_list.commands[0].argv[1]) == -1) {
                        char err_msg[100];
                        sprintf(err_msg, "cd: %s: No such file or directory\n", cmd_list.commands[0].argv[1]);
                        send_message_string(cli_socket, err_msg);
                    } else {
                        send_message_string(cli_socket, "");
                    }
                } else {
                    char* home = getenv("HOME"); // cd without args
                    if (home && chdir(home) == -1) {
                        send_message_string(cli_socket, "cd: could not change to home directory\n");
                    } else {
                        send_message_string(cli_socket, "");
                    }
                }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    Built_In_Cmds cmd_type = rsh_match_command(clist->commands[0].argv[0]);
    if (cmd_type == BI_CMD_EXIT) {
        close(cli_sock);
        return BI_CMD_EXIT;
    } 
    ...
    printf("Parent process: closed all pipe ends\n");
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (WEXITSTATUS(pids_st[i]) == EXIT_SC) {
            printf("Received EXIT_SC from command %d\n", i);
            exit_code = EXIT_SC;
        }
    }
