...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    char *rsp_buff = malloc(SH_CMD_MAX);
    int cli_socket;
    ssize_t io_size;
    int is_eof;
    ...
    while (1) 
    {
        ...
        // fgets input
        if (!fgets(cmd_buff, SH_CMD_MAX - 1, stdin)) {
            break;
        }
        cmd_buff[SH_CMD_MAX - 1] = '\0';
        ...
        while ((io_size = recv(cli_socket, rsp_buff, SH_CMD_MAX, 0)) > 0) {
            printf("%.*s", (int)io_size, rsp_buff);
            if (io_size > 0 && rsp_buff[io_size - 1] == RDSH_EOF_CHAR) {
                break;
            }
        }
