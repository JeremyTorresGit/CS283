...
int get_student(int fd, int id, student_t *s){
    ...
    // read through database and search for student by ID
    while ((read_size = read(fd, &student, STUDENT_RECORD_SIZE)) > 0) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    // initialize student struct and copy input data
    ...
    // move the file pointer to the correct offset for the student ID
    if (lseek(fd, offset, SEEK_SET) < 0) {
        printf(M_ERR_DB_READ);
        return ERR_DB_OP;
    }

    // read through student records
    ...
    // check duplicates
    if (bytes_read == STUDENT_RECORD_SIZE && existing_student.id == id) {
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    }

    // seek back to position and write new record
    if (lseek(fd, offset, SEEK_SET) < 0) {
        printf(M_ERR_DB_READ);
        return ERR_DB_OP;
    }
...
int del_student(int fd, int id){
    ...
    if (tmp_fd == -1) {
        perror("Error: can't open temporary DB file");
        return ERR_DB_FILE;
    }
    ...
    // read the records and copy them to the temporary file
    ...
    if (rename(TMP_DB_FILE, DB_FILE) != 0) {
        perror("Error: can't replace DB file");
        return ERR_DB_OP;
    }
...
void print_student(student_t *s){
    if (s == NULL || s->id == 0 || s->id == DELETED_STUDENT_ID) {
        return;
    }
...
int main(int argc, char *argv[]){
    ...
    switch(opt){
        ...
        case 'f':
            ...
            switch (rc){
                case NO_ERROR:
                    // printf("(Additional line to satisfy the inital test)\n");
                    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA"); // added for header and test
                    print_student(&student);
                    break;
                case SRCH_NOT_FOUND:
                    printf(M_STD_NOT_FND_MSG, id);
